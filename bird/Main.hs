...
data Action
  ...
  | Shoot                       -- ^ Fire a bullet
  | FixObs Double               -- works with a double?
...
-- | Represents a bullet - Custom type
data Bullet
  = NoBullet    -- ^ Not actually a bullet, used for
  | Bullet -- ^^an actually collidable obstacle
    {
      bulTopLeft         :: V2 Double
    , bulBottomRight     :: V2 Double
    } deriving (Eq, Ord, Show)
...
initial =
  ( Model
      ...
      , fixedObstacles    = []
      , bullets      = []
      , bulletPos    = V2 2000 10 -- STart bullet offscreen
      , timeScore    = 0
      , timeSpeed    = 1
      }
...
bulletSpeed :: V2 Double
bulletSpeed = V2 15 0

bulletMaxRange :: Double
bulletMaxRange = 450
...
relevantObs Model { .. } =
  take n obstacles
  -- Where obstacles not in touching bullet
...
touchingObs model@Model { .. } =
  isJust $ find f $ getFixedObs model
...
getFixedObs :: Model -> [Obstacle]
getFixedObs model@Model { .. } =
    ((relevantObs model) \\ fixedObstacles)

bulletTouchObs :: Model -> [Obstacle]
bulletTouchObs model@Model { .. } =
  -- isJust $ find f $ relevantObs model -- Find is for leftmost
  -- f finds objects intersecting bullet
  -- we want to keep objects that DONT, so we inverse it
  (filter f relevantObjectList) -- Filter filters away objs touching bullet
  -- todo update obs ?

  where
    relevantObjectList = relevantObs model

    V2 x y = bulletPos
    V2 w h = bulletDims

    -- The bullet pos is centered. Turn it into a box.
    ftx = x - w / 2
    fty = y - h / 2
    fbx = x + w / 2
    fby = y + h / 2

    -- Check if the bullet box and obs box intersect.
    -- If so, we want to exclude it .
    f NoObstacle = False
    f Obstacle { .. } =
      max tx ftx < min bx fbx &&
      max ty fty < min by fby &&
      playerx - x < bulletMaxRange


      where
        V2 tx ty = obsTopLeft
        V2 bx by = obsBottomRight
        V2 playerx playery = flapperPos
...
canFireBullet :: V2 Double -> V2 Double -> Bool
canFireBullet pos fpos =
    x >= bulletMaxRange + z
...
update model@Model { .. } (Animate dt) =
  ...
  where
    ...
    bulletSpeed' = scrollVel * (bulletSpeed * V2 dt' dt')
    bPos = if dead
           then V2 0 0
           else bulletPos + bulletSpeed

    -- | Hack to update objects list inside player animation loop ((shrug))

    

-- | Should be a subscription to a fire button (Z)
...
update model@Model { .. } (FixObs  dt) =
    if playerStatus == Dead || playerStatus == Waiting || (length (bulletTouchObs model)) == 0
    then (model, Cmd.none)
    else
      ( model
        { fixedObstacles = bulletTouchObs model 
        }
      , Cmd.none
      )
...
subscriptions = Sub.batch
  ...
  , Keyboard.presses $ \key -> (case key of
      ...
      Keyboard.ReturnKey -> Flap
      Keyboard.ZKey       -> Shoot
      _                 -> DoNothing)
  , Time.fps 120 Animate
  , Time.fps 120 FixObs
  ]
...
playingOverlay color Model { .. } =
  ...
  where
    m = canFireBullet bulletPos flapperPos
    V2 posboi _ = bulletPos
    V2 flap _ = flapperPos
    diff = flap - posboi
    status = secondsText timeScore ++ " | " ++ printf "%.2fx speed" timeSpeed ++ "\n ayy lmao" ++ boolToString m ++ "\n" 
        ++ printf "%.2fx pos" diff
    V2 _ h = fromIntegral <$> windowDims
...
view model@Model { .. } = Graphics2D $
  center (V2 (w / 2) (h / 2)) $ collage
    ...
    , toForm $ center (V2 (-x) 0) $ collage
        ...
        , move bulletPos bullet
        , group $ map structure $ getFixedObs model
  ...
  where
    ...
    bullet = filled (rgb 0.36 0.25 0.82) $ rect bulletDims
